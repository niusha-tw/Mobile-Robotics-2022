...
"""
q2.2:  probability of the complete scan,
...
def beam_based_model(z_scan, z_scan_exp, b, z_max):
    ...
    if z_scan > z_max :
      ...
      p_hit = (normalizer(z_scan_exp, b, z_max)* np.exp(-(z_scan - z_scan_exp)**2 / (2*b**2)))/(np.sqrt(2 *b* np.pi))
    return p_hit
...
def main():
    ...
    for i in range(beam_directions.size):
      ...
      # calculate the scan probability for each beam which is indepndent to others , so to get full probability multiply the probability from first index to the last
      prob= prob* beam_based_model(z_scan[i]*100.0, z_scan_exp[i]*100.0, b, z_max*100.0)
    print("the full scan probability is ", prob)
